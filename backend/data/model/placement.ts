...
export class Placement extends Base {
    ...
    static searchable: string[] = ['full_name', 'status', 'address'];
    static fillable: string[] = ['guest_id', 'host_id', 'accommodation_id', 'status', 'start_date', 'end_date'];
    static statuses: string[] = ['active', 'completed'];
    ...
    static async create(row: any): Promise<any> {
        ...
        (guest_id, host_id, accommodation_id, status, start_date, end_date)
        VALUES (?, ?, ?, ?, ?, ?)`,
                [
                    row.guest_id,
                    row.host_id,
    ...
    static async update(partial: any): Promise<any> {
        ...
        try {
            await conn.query(
                `UPDATE ${this.table}
         SET guest_id = ?, host_id = ?, accommodation_id = ?, status = ?,
             start_date = ?, end_date = ?
         WHERE id = ?`,
                [
                    partial.guest_id,
    ...
    static async list(
        ...
        `;
          ...
          p.end_date LIKE ?`;
            ...
            // console.log('Placement List Query:', query); // debug log
